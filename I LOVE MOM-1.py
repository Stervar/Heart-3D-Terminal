...
def draw_heart(points, width=TERMINAL_WIDTH, height=TERMINAL_HEIGHT-15, time_val=0):
    shading_chars = " .:!*OQ#"
    aspect_ratio = width / height
    x = (points[:, 0] / np.max(np.abs(points[:, 0])) * (width//4) + width//2).astype(int)
    y = (points[:, 1] / np.max(np.abs(points[:, 1])) * (height//2 * aspect_ratio) + height//3).astype(int)
    z = points[:, 2]
...
def main():
    ...
    try:
        while True:
            ...
            # Обновляем размеры терминала на каждом кадре
            terminal_size = shutil.get_terminal_size()
            width = terminal_size.columns
            height = terminal_size.lines
            
            frame = draw_heart(rotated_points, width=width, height=height-15, time_val=current_time)
            big_text = draw_big_animated_text(current_time, width)
            
            fps_counter.append(time.time())
            fps = calculate_fps(fps_counter)
            
            # Центрируем статусную строку
            ...
            top_padding = "\n" * ((height - frame_with_status.count("\n") - 2) // 2)
            
            sys.stdout.write('\033[H' + top_padding + frame_with_status)
            sys.stdout.flush()
