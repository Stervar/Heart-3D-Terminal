...
- **Анимация**: Сердце пульсирует и вращается, создавая эффект глубины.
- **Цветовая палитра**: Используется HSV цветовая модель для окраски символов в зависимости от их глубины.
- **Оптимизированный FPS**: Подсчет кадров в секунду для плавной анимации.
- **Кроссплатформенность**: Работает в терминалах, поддерживающих ANSI-экраны.
...
- После запуска программы анимация сердца начнется автоматически.
...
def get_colored_char(char, hue, saturation=1.0, value=1.0):
    ...
    r, g, b = [int(x * 255) for x in colorsys.hsv ```python
_to_rgb(hue, saturation, value)]
